...
public class bancos extends javax.swing.JInternalFrame {
    ...
    private void abm() {
        ...
        switch (operacion) {
            case 1:
                ...
                sql = "select * from banco where descripcion = '" + descripcion.getText().toUpperCase().replace("'", "''") + "'";

                res = db.QueryDinamico(sql, menu.getConexion());

                if (res == null) {
                    String columna[] = {"descripcion", "estado", "usuario_input"};
                    String datos[] = {"'" + descripcion.getText().toUpperCase().replace("'", "''") + "'", String.valueOf(estado.getItemAt(estado.getSelectedIndex()).getId()), String.valueOf(menu.getIduser())};
                    String tbl = "banco";
            ...
            case 2:
                ...
                String datos2[] = {"'" + descripcion.getText().toUpperCase().replace("'", "''") + "'", String.valueOf(estado.getItemAt(estado.getSelectedIndex()).getId()), String.valueOf(menu.getIduser()), "now()"};
                String tbl2 = "banco";
    ...
    private void Selecciona() {
        ...
        if (row < 0) {
            ...
            id = Integer.parseInt(tabla.getValueAt(row, 0).toString());
            sql = "select descripcion, estado from banco where id = " + id;
            res = db.QueryDinamico(sql, menu.getConexion());
