...
public class dias_feriados extends javax.swing.JInternalFrame {
    ...
    private void CargarGrilla() {
        try {
            ...
            Logger.getLogger(dias_feriados.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        try {
            switch (operacion) {
                case 1:
                    ...
                    sql = "select * from dias_feriados where trim(upper(descripcion)) = ?";
    ...
    private void Selecciona() {
        ...
        if (row < 0) {
            ...
            try {
                id = Integer.parseInt(tabla.getValueAt(row, 0).toString());
                sql = "select descripcion, estado from dias_feriados where id = ?";
                PreparedStatement ps = menu.getConexion().prepareStatement(sql);
                ps.setInt(1, id);
