...
public class tipo_empleado extends javax.swing.JInternalFrame {
    ...
    private void CargarGrilla() {
        try {
            ...
            Logger.getLogger(tipo_empleado.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        try {
            switch (operacion) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    sql = "delete from tipo_empleado where id = ?";
    ...
    private void Selecciona() {
        ...
        if (row < 0) {
            ...
            try {
                id = Integer.parseInt(tabla.getValueAt(row, 0).toString());
                sql = "select descripcion, estado from tipo_empleado where id = ?";
                PreparedStatement ps = menu.getConexion().prepareStatement(sql);
                ps.setInt(1, id);
