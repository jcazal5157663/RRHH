...
public class buscador_sector extends javax.swing.JDialog {
    ...
    public buscador_sector(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void CargarGrilla() {
        try {
            ...
            Logger.getLogger(buscador_sector.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        try {
            switch (operacion) {
                case 1:
                    ...
                    sql = "select * from sector where trim(upper(descripcion)) = ?";
    ...
    private void Selecciona() {
        ...
        if (row < 0) {
            ...
            try {
                id = Integer.parseInt(tabla.getValueAt(row, 0).toString());
                sql = "select descripcion, estado from sector where id = ?";
                PreparedStatement ps = menu.getConexion().prepareStatement(sql);
                ps.setInt(1, id);
